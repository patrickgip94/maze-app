...
function addItem({}: Props) {
  ...
  const mintNft = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const tx = await contract.mintTo(address, metadata); // tx = transaction, address = user, metadata = listing
    ...
    <div>
      ...
      <main className="max-w-6xl mx-auto p-10 border">
        ...
        <p className="pb-5">
          ...
          essentially Minting an NFT of the item into your wallet, 
          which we can then list it for sale!
        </p>
        <div className="flex flex-col justify-center items-center md:flex-row md:space-x-5 pt-5">
          ...
          <form onSubmit={mintNft} className="flex flex-col flex-1 p-2 space-y-2">
            ...
            <button type="submit" className="bg-blue-600 font-bold text-white rounded-full py-4 
              px-10 w-56 md:mt-auto mx-auto md:ml-auto">
              Add/Mint Item
            </button>
          </form>
        </div>
